...
function UI.Create()
    ...
    -- 圆圈图片按钮 (ANGELI 风格)
    local circleBtn = mk("ImageButton", {Size = UDim2.new(0, 26, 0, 26), Position = UDim2.new(0, 14, 0.5, -13), BackgroundColor3 = T.accent, BorderSizePixel = 0, Image = "rbxassetid://16014323157", ImageColor3 = Color3.new(1, 1, 1), AutoButtonColor = false, Parent = titleBar})
    Instance.new("UICorner", circleBtn).CornerRadius = UDim.new(1, 0)
    circleBtn.MouseEnter:Connect(function() tw(circleBtn, 0.12, {BackgroundColor3 = T.accentHi}) end)
    circleBtn.MouseLeave:Connect(function() tw(circleBtn, 0.12, {BackgroundColor3 = T.accent}) end)
    
    -- 动漫横幅
    local banner = mk("ImageLabel", {Size = UDim2.new(1, -24, 0, 45), Position = UDim2.new(0, 12, 0, 42), BackgroundTransparency = 1, Image = "rbxassetid://16014323157", ScaleType = Enum.ScaleType.Crop, Parent = main})
    Instance.new("UICorner", banner).CornerRadius = UDim.new(0, 8)
    local bannerStroke = Instance.new("UIStroke", banner)
