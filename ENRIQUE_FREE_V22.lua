...
--[[ENRIQUE FREE | Direct Embedded Runtime v21]]
--[[ENRIQUE FREE | Direct Embedded Runtime v22]]
do
...
local function _report(message)
    message = tostring(message):sub(1, 900)
    warn("[ENRIQUE] " .. message)
    pcall(function()
    ...
    pcall(function()
        local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
            or game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui", 3)
        if not pg then return end
        local old = pg:FindFirstChild("ENRIQUE_ERROR_REPORT")
        if old then old:Destroy() end
        local gui = Instance.new("ScreenGui")
        gui.Name = "ENRIQUE_ERROR_REPORT"
        gui.ResetOnSpawn = false
        gui.IgnoreGuiInset = true
        gui.DisplayOrder = 999999
        gui.Parent = pg
        local frame = Instance.new("Frame")
        frame.Size = UDim2.fromOffset(520, 150)
        ...
        label.TextYAlignment = Enum.TextXAlignment.Top
        label.Font = Enum.Font.Code
        label.TextSize = 14
        label.Text = "[ENRIQUE ERROR]\n" .. message
        label.Parent = frame
    end)
end
...
-- Works on: Synapse X, Krnl, Fluxus, Delta, Codex, Arceus X,
-- Hydrogen, Comet, Vega X, Nihon, Wave, and more
-- ============================================================
...
-- bit32 fallback (manual XOR implementation)
...
if not bit32.lshift then
    bit32.lshift = function(a, b) return (a * (2 ^ b)) % 4294967296 end
end
...
if not bit32.ror then
    ...
    bit32.ror = function(a, b)
        b = b % 32
        if b == 0 then return a end
        return bit32.band(bit32.rshift(a, b), bit32.lshift(a, 32 - b)) + bit32.lshift(bit32.band(a, 2^b - 1), 32 - b)
    end
end

-- hookfunction fallback (no-op if not available)
local hookfunction = hookfunction or function(orig, replacement)
    -- Can't hook, just call original behavior
    return orig
end
...
-- newcclosure fallback
local newcclosure = newcclosure or function(f) return f end
_G.newcclosure = newcclosure

-- setreadonly fallback
local setreadonly = setreadonly or function() end
_G.setreadonly = setreadonly

-- getrawmetatable fallback
local getrawmetatable = getrawmetatable or function(obj) return getmetatable(obj) end
_G.getrawmetatable = getrawmetatable

-- debug.getupvalues fallback
if not debug then debug = {} end
debug.getupvalues = debug.getupvalues or getupvalues or function(f) return {} end

-- islclosure fallback
local islclosure = islclosure or function(f) return type(f) == "function" and not is_luau(f) end
_G.islclosure = islclosure

-- getconnections - wrapped in safe check
local _safeGetConnections = function(obj, signal_name)
    local ok, conns = pcall(function()
        if getconnections then
            return getconnections(obj[signal_name])
        elseif get_signal_cons then
            return get_signal_cons(obj[signal_name])
        end
        return nil
    end)
...
_G._safeGetConnections = _safeGetConnections
...
if not _loadstring then
    error("[ENRIQUE] This executor does not support loadstring")
end

print("[ENRIQUE] Compatibility layer loaded")

-- Optional addon loader; the main UI is embedded below.
local function _load_url(...)
    for _, url in ipairs({...}) do
        local source = _HttpGet(url)
        if type(source) == "string" and #source > 64 then
            local chunk, chunk_error = _loadstring(source, url)
            if chunk then
                local ok, result = pcall(chunk)
                if ok then return result end
                warn("[ENRIQUE] addon failed: " .. tostring(result))
            else
                warn("[ENRIQUE] addon syntax error: " .. tostring(chunk_error))
            end
        end
    end
    return nil
...
-- The UI runtime is embedded directly in this function. It is not recompiled
-- through a second loadstring call, which some executors cannot handle.
local _embedded_ok, Library = pcall(function()
...
local Library = {
    ...
    Scheme = {
        ...
        FontColor = Color3.fromRGB(255, 105, 180),
...
if not _embedded_ok or not Library then
    error("[ENRIQUE] embedded UI failed: " .. tostring(Library))
end
...
-- Create/show the window immediately so a later addon error cannot hide the UI.
local _window_ok, Window = pcall(function()
...
if not _window_ok or not Window then
    _report("UI window creation failed: " .. tostring(Window))
    return
end
...
local _feature_ok, _feature_error = pcall(function()
task.spawn(function()
    task.wait(0.5)
    pcall(function()
...
local Alive = workspace:FindFirstChild("Alive") or workspace:WaitForChild("Alive", 30) or Instance.new("Folder")
...
pcall(function()
    ...
    if LocalPlayer.PlayerGui:FindFirstChild("Hotbar") and LocalPlayer.PlayerGui.Hotbar:FindFirstChild("Block") then
        local block_conns = _safeGetConnections(LocalPlayer.PlayerGui.Hotbar, "Block")
...
if not _feature_ok then
    _report(tostring(_feature_error))
end
