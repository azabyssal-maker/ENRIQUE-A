...
--[[ENRIQUE FREE | Selfcontained Runtime v20]]
do
...
-- The Obsidian-compatible UI runtime is embedded so startup does not depend on HTTP.
local _EmbeddedLibrarySource = [==[
...
local _embedded_ok, Library = pcall(function()
    local chunk = _loadstring(_EmbeddedLibrarySource, "ENRIQUE Embedded UI")
    if not chunk then error("cannot compile embedded UI") end
    return chunk()
end)
