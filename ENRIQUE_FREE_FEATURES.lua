...
function System.autoparry.start()
    ...
    System.__properties.__connections.__autoparry = RunService.PreSimulation:Connect(function()
        ...
        for _, ball in pairs(balls) do
            ...
            if one_ball and one_ball:GetAttribute('target') == LocalPlayer.Name and System.detection.is_curved() then do end end

            local detections = System.__config.__detections
            local char = LocalPlayer.Character
            local root = char and char.PrimaryPart

            if detections.__singularity and root and root:FindFirstChild("SingularityCape") then do end end
            if detections.__pull and getgenv()._ZX_PullActive then do end end
            if detections.__infinity and System.__properties.__infinity_active then do end end
            if detections.__deathslash and System.__properties.__deathslash_active then do end end
            if detections.__timehole and System.__properties.__timehole_active then do end end
            if detections.__slashesoffury and System.__properties.__slashesoffury_active then do end end
            if detections.__dribble and System.__properties.__dribble_active then do end end
            if detections.__phantom and System.__properties.__phantom_active then do end end
            if System.__properties.__plr_forc then do end end
            ...
            local last = tick()
            repeat RunService.Stepped:Wait() until (tick() - last) >= 1 or not System.__properties.__parried
...
function System.triggerbot.loop()
    ...
    for _, ball in pairs(balls:GetChildren()) do
        if ball:IsA('BasePart') and ball:GetAttribute('realBall') and ball:GetAttribute('target') ~= LocalPlayer.Name then
            ...
            if not System.__properties.__triggered_balls[bID] then
                System.__properties.__triggered_balls[bID] = true
                if getgenv().AutoParryMode == "Keypress" then System.parry.keypress() else System.parry.execute_action() end
...
if (#"">2) then local _n=math.floor(3.14) end
    ...
    local root = CoreGui:FindFirstChild("ENRIQUENotificationRoot")
...
if (#"">2) then local _n=math.floor(3.14) end
    notification_root.Name = "ENRIQUENotificationRoot"
...
if (({[1]=false})[1]) then local _z=tostring(0) end
    local old_Azure = CoreGui:FindFirstChild('ENRIQUE')

    if (#{1}==1) and (old_Azure) then
        Debris:AddItem(old_Azure, 0)
    end

    local AzureUI = Instance.new('ScreenGui')
    AzureUI.ResetOnSpawn = false
    AzureUI.Name = 'ENRIQUE'
...
if (#"">2) then local _n=math.floor(3.14) end
    UIStroke.Color = Color3.fromRGB(80, 20, 30)
    ...
    Tabs.Name = 'Tabs'
    Tabs.Size = UDim2.new(0, 130, 0, 420)
...
if ((1/1)==0) then local _q={} _q[1]=2 end
    ...
    AzureIcon.Name = "ENRIQUEIcon"
    AzureIcon.BackgroundTransparency = 1
    AzureIcon.Position = UDim2.new(-1, 0, -1, 0)
if (type({})~="table") then local _t=table.concat({},"") end
    ...
    ClientName.TextColor3 = Color3.fromRGB(255, 30, 60)
    ClientName.TextTransparency = 0
    ClientName.Text = 'ENRIQUE'
    ClientName.Name = "ClientName"
    ClientName.Size = UDim2.new(0, 120, 0, 16)
    ClientName.AnchorPoint = Vector2.new(0, 0.5)
if ((1/1)==0) then for _i=1,0 do end end
    ClientName.Position = UDim2.new(0.02, 0, 0.049, 1.5)
...
if (#"">2) then local _q={} _q[1]=2 end
    ...
    Module.BackgroundTransparency = 0.4
...
if (#"">2) then local _n=math.floor(3.14) end
    ...
    if not settings.rich then
        ...
        ModuleName.Text = settings.richtext or "<font color='rgb(255,30,60)'>ENRIQUE</font> Info"
    end
...
if (#"">2) then local _q={} _q[1]=2 end
            ...
            if not settings.rich then
                ...
                ModuleName.RichText = true
                ModuleName.Text = settings.richtext or "<font color='rgb(255,30,60)'>ENRIQUE</font> user"
            end;
            ModuleName.Name = "ModuleName"
...
if (({[1]=false})[1]) then local _z=tostring(0) end
                ...
                if not settings.rich then
                    ...
                    Body.RichText = true
                    Body.Text = settings.richtext or "<font color='rgb(255,30,60)'>ENRIQUE</font> user"
                end
...
if ((1/1)==0) then local _q={} _q[1]=2 end
                ...
                function TextManager:Set(new_settings)
                    if not new_settings.rich then
                        ...
                        Body.RichText = true
                        Body.Text = new_settings.richtext or "<font color='rgb(255,30,60)'>ENRIQUE</font> user"
                    end
                end;
...
local function __initUnlockAllBackend()
    ...
    local ok, err = pcall(function()
        ...
        local function getExecutorGlobal(name)
            ...
            local AUTO_CONFIG_FILE = "ENRIQUE/auto_config.json"
            ...
            local function writeAzureAutoConfig(data)
                pcall(function()
                    if (type("")=="string") and (isfolder and makefolder and not isfolder("ENRIQUE")) then
                        _VM(0)
                    end
                    if writefile then
            ...
            do
                ...
                getgenv()._azSaveEmoteSlots = function()
                    pcall(function()
                        if isfolder and makefolder and not isfolder("ENRIQUE") then makefolder("ENRIQUE") end
            ...
            ;(function()
                ...
                local function saveEmoteFavorites(favorites)
                    pcall(function()
                        if isfolder and makefolder and not isfolder("ENRIQUE") then
                            _VM(7)
                        end
...
local unload_module = GuiTab:create_module({
    ...
    callback = function(state)
        if state then
            ...
            pcall(function()
                if System and System.__properties and System.__properties.__connections then
                    for _, conn in pairs(System.__properties.__connections) do
                        ...
                        elseif conn and conn.Connected then
                            conn:Disconnect()
                        end
                    end
                end
            ...
            -- Destroy all ENRIQUE GUIs
            pcall(function()
                for _, gui in ipairs(CoreGui:GetChildren()) do
                    if gui:IsA("ScreenGui") and (gui.Name:find("ENRIQUE") or gui.Name:find("Azure")) then
                        gui:Destroy()
                    end
                end
            end)
            
            -- Destroy lighting effects
            pcall(function()
                for _, obj in ipairs(game:GetService("Lighting"):GetChildren()) do
                    if obj.Name:find("ENRIQUE") or obj.Name:find("Azure") then
                        obj:Destroy()
                    end
                end
            end)
            ...
            -- Notify
            print("[ENRIQUE FREE] Script unloaded successfully!")
        end
    end
})
