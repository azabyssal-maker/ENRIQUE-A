--[[
    ╔═══════════════════════════════════════════════════════════╗
    ║          ENRIQUE FREE — PREMIUM KEY SYSTEM v2.0           ║
    ║         Advanced Authentication + Cinematic Loading        ║
    ╚═══════════════════════════════════════════════════════════╝
]]

if _G.__ENRIQUE_ACTIVE then return end
...
-- // Config
local KEY_API = "https://enrique-key-system.vercel.app/api/keys"
local KEY_FILE = "ENQ_KEY.dat"
local SESSION_DURATION = 86400 -- 24 hours

-- // Utility: Safe GUI Parent
...
-- // Utility: Particle System for Background
local function createParticles(parent, count)
    ...
    for i = 1, count do
        local p = new("Frame", {
            ...
            BackgroundColor3 = Color3.fromRGB(255, math.random(20, 60), math.random(40, 80)),
    ...
    conn = RunService.Heartbeat:Connect(function()
        ...
        for _, pt in ipairs(particles) do
            ...
            local newX = pos.X.Scale + pt.speedX
            local newY = pos.Y.Scale + pt.speedY
            if newX < 0 or newX > 1 then pt.speedX = -pt.speedX end
            if newY < 0 or newY > 1 then pt.speedY = -pt.speedY end
            pt.frame.Position = UDim2.fromScale(newX, newY)
            
            local trans = pt.frame.BackgroundTransparency + pt.alpha_speed
            if trans > 0.9 then pt.alpha_speed = -pt.alpha_speed elseif trans < 0.3 then pt.alpha_speed = -pt.alpha_speed end
...
local function shake(frame, intensity)
    ...
    for i = 1, 6 do
        ...
        frame.Position = UDim2.new(orig.X.Scale, orig.X.Offset + offset.X, orig.Y.Scale, orig.Y.Offset + offset.Y)
        task.wait(0.03)
    end
    frame.Position = orig
end
...
-- KEY VALIDATION
-- ============================================================
local function generateSessionToken(key)
    local timestamp = tostring(os.time())
    local combined = key .. "|" .. timestamp .. "|" .. (LocalPlayer and LocalPlayer.UserId or 0)
    local hash = 0
    for i = 1, #combined do
...
local function isValidKeyFormat(key)
    ...
    for i = 1, #key do
        ...
        if i == 5 or i == 10 or i == 15 then
            ...
            if not (c:match("%d") or c:match("[%u]")) then return false end
        end
    end
    return true
end
...
local function validateKey(key)
    -- Known valid keys (offline validation)
    local knownKeys = {
        ["NX7K-9M2P-QW4R-TY8V"] = {tier = "Premium", expires = nil},
        ["B3NQ-L8XD-FH6J-ZP1M"] = {tier = "VIP", expires = nil},
        ["RT5W-KC9Y-VN2B-JD7S"] = {tier = "Premium", expires = nil},
        ["AAAA-BBBB-CCCC-DDDD"] = {tier = "Owner", expires = nil},
        ["XXXX-XXXX-XXXX-XXXX"] = {tier = "Master", expires = nil},
    }
    
    -- Also try online API (optional)
    local onlineResult = nil
    pcall(function()
        local response = game:HttpGet(KEY_API .. "?key=" .. key)
        if response then
    ...
    if onlineResult then return true, onlineResult.tier or "Online" end
    if knownKeys[key] then return true, knownKeys[key].tier end
...
local function showLoading(onComplete)
    local gui = new("ScreenGui", {Name = "ENQ_Cinematic", ResetOnSpawn = false, IgnoreGuiInset = true, DisplayOrder = 99999})
    safeParent(gui)
    
    local bg = new("Frame", {Size = UDim2.fromScale(1, 1), BackgroundColor3 = Color3.fromRGB(3, 3, 8), BorderSizePixel = 0, Parent = gui})
    addGradient(bg, 160, Color3.fromRGB(8, 3, 12), Color3.fromRGB(2, 2, 5))
    
    -- Particles
    createParticles(bg, 25)
    ...
    local ringStroke = new("UIStroke", {Color = Color3.fromRGB(255, 30, 60), Thickness = 2, Transparency = 0.4, Parent = ring})
    ...
    local barGlow = new("UIStroke", {Color = Color3.fromRGB(255, 30, 60), Thickness = 1, Transparency = 0.7, Parent = barContainer})
    
    local barFill = new("Frame", {
        Size = UDim2.fromScale(0, 1),
        BackgroundColor3 = Color3.fromRGB(255, 30, 60),
        BorderSizePixel = 0,
        Parent = barContainer
    })
    Instance.new("UICorner", barFill).CornerRadius = UDim.new(1, 0)
    
    local barGradient = new("UIGradient", {Parent = barFill})
    barGradient.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, Color3.fromRGB(180, 20, 45)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 60, 90))
    }
    
    -- Percentage text
    local pctText = new("TextLabel", {
        AnchorPoint = Vector2.new(0.5, 0),
    ...
    task.spawn(function()
        -- Logo glow pulse
        while gui.Parent do
            TweenService:Create(glow, TweenInfo.new(1.2, Enum.EasingStyle.Sine), {ImageTransparency = 0.85}):Play()
            task.wait(1.2)
            TweenService:Create(glow, TweenInfo.new(1.2, Enum.EasingStyle.Sine), {ImageTransparency = 0.55}):Play()
            task.wait(1.2)
        end
    end)
    ...
    -- Typewriter title
    typewriter(title, "ENRIQUE FREE", 0.06)
    task.delay(0.8, function()
        typewriter(subtitle, "Advanced Blade Ball Solution", 0.04)
    end)
    ...
    task.spawn(function()
        local steps = {
            ...
            {"Establishing secure connection...", 30},
            {"Loading combat modules...", 50},
            {"Optimizing performance...", 70},
            {"Applying bypass patches...", 88},
            {"Finalizing...", 100}
        }
        
        for _, step in ipairs(steps) do
            subtitle.Text = step[1]
            local targetProgress = step[2] / 100
            
            -- Smooth progress animation
            local startProgress = tonumber(barFill.Size.X.Scale) or 0
            local progressTween = TweenService:Create(barFill, TweenInfo.new(0.8, Enum.EasingStyle.Quart), {
                Size = UDim2.fromScale(targetProgress, 1)
            })
            progressTween:Play()
            ...
            task.spawn(function()
                for t = 0, 1, 0.02 do
                    local current = startProgress + (targetProgress - startProgress) * t
                    pctText.Text = math.floor(current * 100) .. "%"
                    task.wait(0.016)
                end
                pctText.Text = targetProgress * 100 .. "%"
            end)
...
local function showKeySystem(onSuccess)
    local gui = new("ScreenGui", {Name = "ENQ_PremiumKey", ResetOnSpawn = false, IgnoreGuiInset = true, DisplayOrder = 99998})
    safeParent(gui)
    ...
    createParticles(overlay, 15)
    
    -- Main card
    local card = new("Frame", {
        AnchorPoint = Vector2.new(0.5, 0.5),
        Position = UDim2.fromScale(0.5, 0.5),
        Size = UDim2.fromOffset(0, 0), -- Start at 0 for animation
    ...
    local cardStroke = new("UIStroke", {Color = Color3.fromRGB(255, 30, 60), Thickness = 1.5, Transparency = 0.2, Parent = card})
    addGradient(card, 145, Color3.fromRGB(22, 12, 28), Color3.fromRGB(8, 5, 14))
    ...
    local accentGrad = new("UIGradient", {Parent = accentLine})
    accentGrad.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 30, 60)),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(150, 20, 40)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 30, 60))
    }
    ...
    local desc = new("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Text = "Enter your access key to authenticate",
    ...
    local inputContainer = new("Frame", {
        ...
        BackgroundColor3 = Color3.fromRGB(18, 15, 25),
        BorderSizePixel = 0,
        Parent = card
    })
    Instance.new("UICorner", inputContainer).CornerRadius = UDim.new(0, 10)
    
    local inputStroke = new("UIStroke", {Color = Color3.fromRGB(50, 40, 65), Thickness = 1, Parent = inputContainer})
    ...
    local input = new("TextBox", {
        ...
        PlaceholderColor3 = Color3.fromRGB(50, 45, 65),
        Text = "",
    ...
    local btnGrad = new("UIGradient", {Rotation = 90, Parent = submitBtn})
    btnGrad.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 45, 75)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(200, 20, 50))
    }
    ...
    local getKeyLink = new("TextButton", {
        AnchorPoint = Vector2.new(0.5, 0.5),
        Position = UDim2.fromScale(0.5, 0.5),
        Size = UDim2.new(1, 0, 0, 16),
        BackgroundTransparency = 1,
    ...
    -- Glow pulse on card stroke
    task.spawn(function()
        while gui.Parent do
            TweenService:Create(cardStroke, TweenInfo.new(1.5, Enum.EasingStyle.Sine), {Transparency = 0.5}):Play()
            task.wait(1.5)
            TweenService:Create(cardStroke, TweenInfo.new(1.5, Enum.EasingStyle.Sine), {Transparency = 0.15}):Play()
            task.wait(1.5)
        end
    end)
    ...
    task.spawn(function()
        while gui.Parent do
            TweenService:Create(iconGlow, TweenInfo.new(1.5, Enum.EasingStyle.Sine), {ImageTransparency = 0.85}):Play()
            task.wait(1.5)
            TweenService:Create(iconGlow, TweenInfo.new(1.5, Enum.EasingStyle.Sine), {ImageTransparency = 0.6}):Play()
            task.wait(1.5)
        end
    end)
    ...
    submitBtn.MouseEnter:Connect(function()
        TweenService:Create(submitBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(255, 60, 90)}):Play()
    end)
    submitBtn.MouseLeave:Connect(function()
        TweenService:Create(submitBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(255, 30, 60)}):Play()
    end)
    
    -- Input focus effects
    input.Focused:Connect(function()
        TweenService:Create(inputStroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(255, 30, 60), Thickness = 1.5}):Play()
    end)
    input.FocusLost:Connect(function()
        TweenService:Create(inputStroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(50, 40, 65), Thickness = 1}):Play()
    end)
    ...
    local function attemptVerify()
        ...
        task.wait(0.8) -- Simulate API call delay
        
        local valid, tier = validateKey(key)
...
local Config = setmetatable({
    save = function(self: any, file_name: any, config: any)
        local success_save, result = pcall(function()
            ...
            writefile('ENRIQUE/'..file_name..".json", flags)
        end)
    ...
    load = function(self: any, file_name: any, config: any)
        local success_load, result = pcall(function()
            if not isfile('ENRIQUE/'..file_name..".json") then
                self:save(file_name, config)

                return
            end

            local flags = readfile('ENRIQUE/'..file_name..".json")
