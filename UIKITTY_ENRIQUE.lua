...
if not isfolder('ENRIQUE') then
	makefolder('ENRIQUE')
end

if not isfolder('ENRIQUE/assets') then
	makefolder('ENRIQUE/assets')
end
...
function library._init(self: _runtime)
	...
	if get_custom_asset and self._type == '_limited' then
		_pcall(function()
			local cached = isfile('ENRIQUE/assets/anime.png') and readfile('ENRIQUE/assets/anime.png') or nil

			if not cached or cached:sub(2, 4) ~= 'PNG' then
				local content =
					game:HttpGet('https://raw.githubusercontent.com/retilin/Angeli/refs/heads/main/assets/anime.png')

				if content and content:sub(2, 4) == 'PNG' then
					writefile('ENRIQUE/assets/anime.png', content)
				end
			end
		end)

		local logo_loaded, logo_asset = _pcall(function()
			return get_custom_asset('ENRIQUE/assets/anime.png')
		end)
