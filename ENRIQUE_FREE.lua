--[[
    ENRIQUE FREE V55
    Full Blade Ball Script
    Features: Auto Parry, Auto Spam, Manual Spam, Trigger Bot,
              Ability Detection, Skin Changer, Unlock All
    UI: Uikitty with ENRIQUE FREE branding + Anime background
...
function library._init(self: _runtime)
	...
	create_new('ImageLabel', {
		...
		Image = 'rbxassetid://86955036432319',
	...
	local user_info = create_new('Frame', {
		Name = '_user_info',
		BackgroundColor3 = Color3.fromRGB(0, 0, 0),
		AnchorPoint = _new_vector2(0, 1),
		Position = _new_udim2(0, 10, 1, -10),
		Size = _new_udim2(1, -20, 0, 44),
		BorderSizePixel = 0,
		ZIndex = 20,
		Parent = sidebar,
	})

	create_round(user_info, 7)
	create_outline(user_info)

	local players_service = game:GetService('Players')
	local local_player = players_service.LocalPlayer

	local user_avatar = create_new('ImageLabel', {
		BackgroundColor3 = Color3.fromRGB(18, 18, 18),
		BackgroundTransparency = 0,
		AnchorPoint = _new_vector2(0, 0.5),
		Position = _new_udim2(0, 6, 0.5, 0),
		Size = _new_udim2(0, 32, 0, 32),
		Image = players_service:GetUserThumbnailAsync(local_player.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size420x420),
		BorderSizePixel = 0,
		ZIndex = 21,
		Parent = user_info,
	})

	create_pill(user_avatar)

	local user_name = create_label({
		AnchorPoint = _new_vector2(0, 0.5),
		Position = _new_udim2(0, 46, 0.5, 0),
		Size = _new_udim2(1, -52, 0, 18),
	...
	create_new('ImageLabel', {
		...
		Image = 'rbxassetid://92296849858473',
		ImageColor3 = Color3.fromRGB(255, 255, 255),
	...
	local status_frame = create_new('Frame', {
		Name = '_status_bar',
		AnchorPoint = _new_vector2(1, 0.5),
		Position = _new_udim2(1, -14, 0.5, 0),
		Size = _new_udim2(0, 150, 0, 28),
	...
	local status_label = create_label({
		Name = '_status',
		AnchorPoint = _new_vector2(0.5, 0.5),
		Position = _new_udim2(0.5, 0, 0.5, 0),
		Size = _new_udim2(1, -12, 1, 0),
	...
	RunService.RenderStepped:Connect(function()
		...
		if elapsed >= 0.5 then
			current_fps = math.floor(frame_count / elapsed + 0.5)
			frame_count = 0
			last_frame = now
		end
	end)
	...
	local function get_ping()
		...
		if ok and _type(value) == 'number' and value >= 0 then
			return math.floor(value * 1000 + 0.5)
		end

		local stats_ok, stats_value = _pcall(function()
			local network = game:GetService('Stats').Network
			local data_ping = network.ServerStatsItem['Data Ping']
			return data_ping:GetValue()
		end)

		if stats_ok and _type(stats_value) == 'number' then
			return math.floor(stats_value + 0.5)
		end
	...
	local function update_status()
		local now = os.date('*t')
		status_label.Text = string.format('FPS: %d | Ping: %d ms', current_fps, get_ping())
	end

	task.spawn(function()
		while topbar.Parent and sidebar.Parent do
			update_status()
			task.wait(0.5)
		end
	end)
	...
	if get_custom_asset and self._type == '_limited' then
		...
		if logo_loaded and logo_asset and logo_asset ~= '' then
			create_new('ImageLabel', {
				BackgroundTransparency = 1,
				AnchorPoint = _new_vector2(1, 0.5),
				Position = _new_udim2(1, -116, 0.5, 0),
	...
	function self:change_visibility(state)
		if not self._fade_objects then
			self._fade_objects = {}

			for _, object in container:GetDescendants() do
				if object:IsA('Frame') or object:IsA('TextLabel') or object:IsA('TextButton') or object:IsA('ImageLabel') or object:IsA('ImageButton') or object:IsA('UIStroke') then
					table.insert(self._fade_objects, {
						object = object,
						background = object:IsA('Frame') and object.BackgroundTransparency or nil,
						text = (object:IsA('TextLabel') or object:IsA('TextButton')) and object.TextTransparency or nil,
						image = (object:IsA('ImageLabel') or object:IsA('ImageButton')) and object.ImageTransparency or nil,
						stroke = object:IsA('UIStroke') and object.Transparency or nil,
					})
				end
			end
		end
		...
		if state then
			...
			for _, data in self._fade_objects do
				...
				local properties = {}
				if data.background ~= nil then properties.BackgroundTransparency = data.background end
				if data.text ~= nil then properties.TextTransparency = data.text end
				if data.image ~= nil then properties.ImageTransparency = data.image end
				if data.stroke ~= nil then properties.Transparency = data.stroke end

				if next(properties) then
					_create_tween(
						tween_service,
						object,
						_new_tween_info(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
						properties
					):Play()
				end
			end
			...
			for _, data in self._fade_objects do
				...
				local properties = {}
				if data.background ~= nil then properties.BackgroundTransparency = 1 end
				if data.text ~= nil then properties.TextTransparency = 1 end
				if data.image ~= nil then properties.ImageTransparency = 1 end
				if data.stroke ~= nil then properties.Transparency = 1 end
...
function library.create_tab(self: _runtime, title: string, icon: string?, layout_order: number?, category_name: string?)
	...
	function tab_manager:create_group(title, side)
		...
		function group_manager:create_colorpicker(flag, _values)
			...
			local default_color = _values.default or _values.Default or Color3.fromRGB(255, 255, 255)
			local saved_color = runtime._flags[flag]
			local state = typeof(saved_color) == 'Color3' and saved_color or default_color
			...
			local label = create_label({
				Size = _new_udim2(1, -44, 1, 0),
				Text = _values.title or flag,
				TextSize = runtime._label_text_size,
				TextXAlignment = Enum.TextXAlignment.Left,
				TextYAlignment = Enum.TextYAlignment.Center,
				Parent = row,
			...
			local swatch = create_new('TextButton', {
				AnchorPoint = _new_vector2(1, 0.5),
				Position = _new_udim2(1, 0, 0.5, 0),
				Size = _new_udim2(0, 34, 0, 22),
			...
			create_round(swatch, 6)
			create_outline(swatch)

			local picker = create_new('Frame', {
				Visible = false,
				AnchorPoint = _new_vector2(1, 0),
				Position = _new_udim2(1, -4, 0, 32),
				Size = _udim2_from_offset(220, 250),
				BackgroundColor3 = Color3.fromRGB(22, 22, 22),
				BorderSizePixel = 0,
				ZIndex = 200,
				Parent = runtime._ui,
			...
			create_round(picker, 8)
			create_outline(picker)

			local header = create_new('TextButton', {
				BackgroundTransparency = 1,
				Position = _new_udim2(0, 8, 0, 6),
				Size = _new_udim2(1, -16, 0, 24),
				Text = _values.title or flag,
			...
			local sv = create_new('Frame', {
				Position = _new_udim2(0, 10, 0, 38),
				Size = _new_udim2(1, -38, 0, 170),
				BackgroundColor3 = state,
				BorderSizePixel = 0,
				Active = true,
				ZIndex = 201,
			...
			local white_gradient = create_new('UIGradient', {
			...
			local black_gradient = create_new('UIGradient', {
			...
			local hue_bar = create_new('Frame', {
				Position = _new_udim2(1, -20, 0, 38),
				Size = _new_udim2(0, 10, 0, 170),
				BorderSizePixel = 0,
				ZIndex = 201,
				Parent = picker,
			})

			create_round(hue_bar, 5)
			...
			local sv_cursor = create_new('Frame', {
				Size = _udim2_from_offset(8, 8),
				AnchorPoint = _new_vector2(0.5, 0.5),
				BackgroundColor3 = Color3.fromRGB(255, 255, 255),
				BorderSizePixel = 2,
			...
			create_pill(sv_cursor)

			local hue_cursor = create_new('Frame', {
				Size = _udim2_from_offset(18, 4),
				AnchorPoint = _new_vector2(0.5, 0.5),
				BackgroundColor3 = Color3.fromRGB(255, 255, 255),
				BorderSizePixel = 1,
				BorderColor3 = Color3.fromRGB(0, 0, 0),
			...
			local hex = create_label({
				Position = _new_udim2(0, 10, 0, 216),
				Size = _new_udim2(1, -20, 0, 24),
				Text = '#FFFFFF',
				TextSize = runtime._small_text_size + 1,
				TextColor3 = Color3.fromRGB(220, 220, 220),
				TextXAlignment = Enum.TextXAlignment.Center,
			...
			local hue, saturation, value = Color3.toHSV(state)
			local dragging_sv = false
			local dragging_hue = false
			local dragging_picker = false
			local picker_drag_offset = Vector2.zero
			...
			local function update_visuals()
				...
				sv_cursor.Position = UDim2.new(saturation, 0, 1 - value, 0)
				...
				runtime._flags[flag] = color

				if _values.callback then
					_values.callback(color)
				end
			end

			local function set_from_sv(input)
				local p = sv.AbsolutePosition
				local s = sv.AbsoluteSize
				if s.X <= 0 or s.Y <= 0 then return end
				saturation = _clamp((input.Position.X - p.X) / s.X, 0, 1)
				value = 1 - _clamp((input.Position.Y - p.Y) / s.Y, 0, 1)
				update_visuals()
			end

			local function set_from_hue(input)
				local p = hue_bar.AbsolutePosition
				local s = hue_bar.AbsoluteSize
				if s.Y <= 0 then return end
				hue = _clamp((input.Position.Y - p.Y) / s.Y, 0, 1)
				update_visuals()
			end
			...
			user_input_service.InputChanged:Connect(function(input)
				if dragging_sv and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
					set_from_sv(input)
				elseif dragging_hue and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
					set_from_hue(input)
				elseif dragging_picker and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
					local mouse = input.Position
					picker.Position = UDim2.fromOffset(
						mouse.X - picker_drag_offset.X,
						mouse.Y - picker_drag_offset.Y
					)
				end
			end)
			...
			header.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1
					...
					picker_drag_offset = input.Position - picker.AbsolutePosition
				end
			end)
			...
			user_input_service.InputBegan:Connect(function(input)
				...
				local s = picker.AbsoluteSize

				local inside_picker =
					position.X >= p.X and position.X <= p.X + s.X
					and position.Y >= p.Y and position.Y <= p.Y + s.Y

				local swatch_position = swatch.AbsolutePosition
				local swatch_size = swatch.AbsoluteSize
				local inside_swatch =
					position.X >= swatch_position.X and position.X <= swatch_position.X + swatch_size.X
					and position.Y >= swatch_position.Y and position.Y <= swatch_position.Y + swatch_size.Y

				if not inside_picker and not inside_swatch then
...
local lua_store_path = LPH_NO_VIRTUALIZE(function()
	return `Angeli/luas/{game.GameId}.json`
end)
...
local read_lua_store = LPH_JIT(function()
	...
	if _type(listfiles) == 'function' then
		local list_success, paths = _pcall(listfiles, 'Angeli/luas')
...
local build_lua_environment = LPH_ENCFUNC(function(queued)
	...
	local custom_require = LPH_NO_UPVALUES(function(module_name)
		if _type(module_name) ~= 'string' then
			error('Angeli Lua require expects a string path', 2)
		end
...
library._load_lua_entry = LPH_ENCFUNC(function(self: _runtime, manager, entry)
	...
	local chunk, compile_error = compile_lua_source(entry.source, `@Angeli/{entry.name}`)
...
function library.load_manager(self: runtime_typeof)
	...
	local manager = {
		...
		_auto_load_files = {},
		_selected = nil,
		_auto_load = self._flags.__angeli_lua_auto_load == true,
	}
	...
	local selector = lua_group:create_dropdown('__angeli_lua_selected', {
		title = 'Luas',
		options = lua_files(),
	...
	local function import_pasted_source()
		...
		local chunk, compile_error = compile_lua_source(source, '@Angeli/import')
	...
	lua_group:create_toggle('__angeli_lua_auto_load', {
		title = 'Auto Load',
		default = false,
		callback = function(state)
    ...
    pcall(function()
        ...
        task.spawn(function()
            ...
            while bg and bg.Parent do
                ...
                bg.ImageTransparency = 0.82 + math.sin(tick() * 0.5) * 0.05
                task.wait(0.05)
            end
        end)
...
local Unlock = VisualCat:create_tab("Unlock", "rbxassetid://lock")
local World = VisualCat:create_tab("World", "rbxassetid://globe")
local WorldPresets = {
    {
        Name="Sunset",
...
local HitSoundsGroup = ESP:create_group("Hit Sounds", "left")

HitSoundsGroup:create_toggle("hit_sounds", {
    title = "Hit Sounds",
    default = false,
    callback = function(state)
...
HitSoundsGroup:create_slider("hit_sound_volume", {
    title = "Volume",
    minimum = 1,
    maximum = 10,
    default = 5,
...
HitSoundsGroup:create_dropdown("hit_sound_type", {
    title = "Hit Sound Type",
    options = HitSoundOptions,
    default = HitSoundOptions[1],
    callback = function(value)
        local id = HitSoundIds[value]
        if id then
            HitSound.SoundId = id
        end
    end,
...
local ParrySuccessRemote = ReplicatedStorage.Remotes:FindFirstChild("ParrySuccess")
...
local BallTrailRainbowEnabled = false
local BallTrailParticleEnabled = false
local BallTrailGlowEnabled = false
local BallTrailColor = Color3.new(1, 1, 1)
local BallTrailState = {}
local RainbowHue = 0

local function clearBallTrail(ball)
    if not ball then
        return
    end
    for _, name in ipairs({"Trail", "ParticleEmitter", "BallGlow", "Attachment0", "Attachment1"}) do
...
local function applyBallTrail(ball)
    if not ball or not ball:IsA("BasePart") then
        return
    end

    if not BallTrailEnabled then
        clearBallTrail(ball)
        return
    end

    local trail = ball:FindFirstChild("Trail")
    if BallTrailState[ball] and trail then
        if BallTrailRainbowEnabled then
    ...
    local oldTrail = ball:FindFirstChild("Trail")
    if oldTrail then
        oldTrail:Destroy()
    end
    ...
    trail.Color = ColorSequence.new(BallTrailRainbowEnabled and Color3.fromHSV(RainbowHue / 360, 1, 1) or BallTrailColor)
    trail.Parent = ball

    if BallTrailParticleEnabled then
        local emitter = Instance.new("ParticleEmitter")
...
local BallTrailGroup = ESP:create_group("Ball Trail", "right")

BallTrailGroup:create_toggle("ball_trail", {
    title = "Ball Trail",
    default = false,
    callback = function(state)
...
BallTrailGroup:create_colorpicker("ball_trail_color", {
    title = "Ball Trail Color",
    default = BallTrailColor,
    callback = function(color)
        BallTrailColor = color
        if not BallTrailRainbowEnabled then
            local balls = workspace:FindFirstChild("Balls")
            if balls then
                for _, ball in ipairs(balls:GetChildren()) do
                    local trail = ball:FindFirstChild("Trail")
...
BallTrailGroup:create_toggle("ball_trail_rainbow", {
    title = "Rainbow Trail",
    default = false,
    callback = function(state)
        BallTrailRainbowEnabled = state
    end,
})

BallTrailGroup:create_toggle("ball_trail_particle", {
    title = "Particle Emitter",
    default = false,
    callback = function(state)
        BallTrailParticleEnabled = state
        if not BallTrailEnabled then
            return
        end
        local balls = workspace:FindFirstChild("Balls")
...
BallTrailGroup:create_toggle("ball_trail_glow", {
    title = "Glow Effect",
    default = false,
    callback = function(state)
        BallTrailGlowEnabled = state
        if not BallTrailEnabled then
            return
        end
        local balls = workspace:FindFirstChild("Balls")
...
DetectionLeft:create_toggle("detection_infinity", {
    ...
    default = Det.infinityEnabled,
    callback = function(value) Det.infinityEnabled = value end,
})
DetectionLeft:create_toggle("detection_deathslash", {
    title = "Death Slash Detection",
    default = Det.deathslashEnabled,
    callback = function(value) Det.deathslashEnabled = value end,
})
DetectionLeft:create_toggle("detection_singularity", {
    title = "Singularity Detection",
    default = Det.singularityEnabled,
    callback = function(value) Det.singularityEnabled = value end,
})
DetectionLeft:create_toggle("detection_forcefield", {
    title = "Forcefield Detection",
    default = Det.forcefieldEnabled,
    callback = function(value) Det.forcefieldEnabled = value end,
})
DetectionLeft:create_toggle("detection_phantom", {
    title = "Phantom Detection",
    default = Det.phantomEnabled,
    callback = function(value) Det.phantomEnabled = value end,
})
DetectionLeft:create_toggle("detection_dribble", {
    title = "Dribble Detection",
    default = Det.dribbleEnabled,
    callback = function(value) Det.dribbleEnabled = value end,
})

local DetectionRight = Detection:create_group("Combat Detection", "right")
DetectionRight:create_toggle("detection_timehole", {
    title = "Time Hole Detection",
    default = Det.timeholeEnabled,
    callback = function(value) Det.timeholeEnabled = value end,
})
DetectionRight:create_toggle("detection_pull", {
    title = "Pull Detection",
    default = Det.pullEnabled,
    callback = function(value) Det.pullEnabled = value end,
})
local SlashesGroup = DetectionRight
SlashesGroup:create_slider("detection_slashes_delay", {
    title = "Parry Delay",
    minimum = 0.05,
    maximum = 0.25,
    default = Det.slashesParryDelay,
    rounding = false,
    callback = function(value) Det.slashesParryDelay = value end,
})
SlashesGroup:create_slider("detection_slashes_count", {
    title = "Max Parry Count",
    minimum = 1,
    maximum = 36,
    default = Det.slashesMaxCount,
    rounding = true,
    callback = function(value) Det.slashesMaxCount = value end,
})
local StaffAction = "Notification"
DetectionRight:create_dropdown("staff_action", {
    title = "Action Mode",
    options = {"Notification", "Kick"},
    default = StaffAction,
    callback = function(value) StaffAction = value end,
})
    ...
    StaffConn=Players.PlayerAdded:Connect(function(plr)
        ...
        task.spawn(function()
            ...
            if StaffAction=="Kick" then
                ...
                pcall(function()
                    game:GetService("StarterGui"):SetCore("SendNotification",{Title="Staff Detected",Text=plr.Name.." joined the server",Duration=6})
                end)
            end
        end)
    end)
end
DetectionRight:create_toggle("staff_detection_active", {
    title = "Staff Monitor",
    default = false,
    callback = function(state)
        getgenv().StaffDetection=state
        if state then StartStaffDetection() else StopStaffDetection() end
    end,
})

local ESPLeft = ESP:create_group("Ability ESP", "left")
do
    local ESP_Data = {}
    local ESP_HeartbeatConn
    local ESP_PlayerAddedConn
    local ESP_Cooldowns = {}
    local ESP_IconCache = {}
    local ESP_CharConns = {}
    local ESP_AliveAddedConn
    local ESP_AliveRemovedConn
    local esp_last_update = 0
    ...
    local function create_esp_for_player(plr)
        task.spawn(function()
            ...
            if ESP_Data[plr] then
                pcall(function() ESP_Data[plr].bill:Destroy() end)
                if ESP_Data[plr].cdConn then pcall(function() ESP_Data[plr].cdConn:Disconnect() end) end
                ESP_Data[plr]=nil
            end
            ...
            local cdConn=char.AttributeChanged:Connect(function(attr)
                if attr=="CooldownExpiration" then
                    ...
                    if (val==0 or val==nil) and tick()-last_cd>0.5 then
                        ...
                        if dur then last_cd=tick(); ESP_Cooldowns[plr]={expiry=tick()+dur,duration=dur} end
                    end
            ...
            ESP_Data[plr]={bill=bill,label=label,icon=icon,timerLabel=timerLabel,cdConn=cdConn,isPassive=ok and type(dur)=="number" and dur<=0}
        end)
    end

    local function add_esp_player(plr)
        if plr==player or ESP_CharConns[plr] then return end
        local a=plr.CharacterAdded:Connect(function()
            ESP_Cooldowns[plr]=nil
            create_esp_for_player(plr)
        end)
        local r=plr.CharacterRemoving:Connect(function()
            ESP_Cooldowns[plr]=nil
        end)
        ESP_CharConns[plr]={a,r}
        if plr.Character then create_esp_for_player(plr) end
    end

    local function stop_ability_esp()
        getgenv().AbilityESP=false
        if ESP_HeartbeatConn then ESP_HeartbeatConn:Disconnect(); ESP_HeartbeatConn=nil end
        if ESP_PlayerAddedConn then ESP_PlayerAddedConn:Disconnect(); ESP_PlayerAddedConn=nil end
        if ESP_AliveAddedConn then ESP_AliveAddedConn:Disconnect(); ESP_AliveAddedConn=nil end
        if ESP_AliveRemovedConn then ESP_AliveRemovedConn:Disconnect(); ESP_AliveRemovedConn=nil end
    ...
    local function start_ability_esp()
        ...
        if not ESP_HeartbeatConn then
            ESP_HeartbeatConn=RunService.Heartbeat:Connect(function()
                if not getgenv().AbilityESP then return end
                local now=tick()
                if now-esp_last_update<0.1 then return end
                esp_last_update=now
        ...
        if not ESP_PlayerAddedConn then
            ESP_PlayerAddedConn=Players.PlayerAdded:Connect(function(plr)
                if getgenv().AbilityESP then add_esp_player(plr) end
            end)
        end
    end

    getgenv()._ENRIQUE_AbilityESP_Stop=stop_ability_esp
    ESPLeft:create_toggle("ability_esp", {
        title="Ability ESP",
        default=false,
        callback=function(state)
...
local ESPRight = ESP:create_group("Ball Statistic", "right")
do
    ...
    ESPRight:create_toggle("ball_statistic", {
        title="Ball Statistic",
        default=false,
        callback=function(state)
            if state then enable_ball_stats() else destroy_ball_stats() end
        end,
    })
end

print("[ENRIQUE] Loaded. Cho 1 cu parry THAT de capture mau, sau do bat Auto Parry va Auto Spam.")

local ParryRight = Parry:create_group("Manual Spam", "right")

local manualSpamEnabled = false
local manualSpamRate = 500
local manualSpamLoop = nil
